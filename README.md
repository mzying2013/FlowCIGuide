...
4. 注册[**fir.im**](https://fir.im/)账号，并获取**API Token**（不收费，不过要手机号码接收短信验证码）![](https://raw.githubusercontent.com/mzying2013/FlowCIGuide/master/10_fir_APITOKEN.png)
...
4. 构建后。发布构建的APP到fir.im平台。首先需要点击**工作流**中左边的"+"号并搜索fir，添加**fir.im上传插件**。在**$FIR_API_TOKEN**框中填入**准备工作4**获取的**API Token**。如果一切顺利的话，你会在fir.im - 我的应用里面看到构建成功的APP包了。赶紧分享你的APP二维码吧。
